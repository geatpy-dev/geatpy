...
def q_sorted_new_templet(AIM_M, AIM_F, PUN_M, PUN_F, FieldDR, problem, maxormin, MAXGEN, MAXSIZE, NIND, SUBPOP, GGAP, selectStyle, recombinStyle, recopt, pm, distribute, drawing = 1):
...
"""
    ...
    [FitnV, frontIdx] = ga.ndominfast(maxormin * ObjV, exIdx) # 快速非支配排序，一定要传入FitnV从而对排除考虑不满足约束条件的解
    ...
    for gen in range(MAXGEN):
        ...
        FitnV = np.ones((ObjV.shape[0], 1)) # 初始化FitnV(仅是为了使调用罚函数时符合罚函数定义的参数列表)
        if PUN_F is not None:
