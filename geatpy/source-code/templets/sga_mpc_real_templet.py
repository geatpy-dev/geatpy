...
def sga_mpc_real_templet(AIM_M, AIM_F, PUN_M, PUN_F, FieldDRs, problem, maxormin, MAXGEN, NIND, SUBPOP, GGAP, selectStyle, recombinStyle, recopt, pm, drawing = 1):
...
"""
    ...
    for gen in range(MAXGEN):
        allObjVs = np.zeros((1, 0)).T # 存储所有子种群合并的目标函数值
        allChroms = np.zeros((NVAR, 0)).T # 存储所有子种群合并的染色体
        if problem == 'R':
            for index in range(len(FieldDRs)):
                Chrom = Chroms[index]
                # 进行遗传算子，生成子代
                ...
                allChrom = np.vstack([Chrom, SelCh]) # 父子合并
                allChroms = np.vstack([allChroms, allChrom])
                allObjVs = np.vstack([allObjVs, aimfuc(allChrom)])
            ...
            for index in range(len(FieldDRs)):
                ...
                allChroms = np.vstack([allChroms, allChrom])
                allObjVs = np.vstack([allObjVs, aimfuc(allChrom)])
        allFitnVs = ga.ranking(maxormin * allObjVs, None, SUBPOP) # 计算所有子种群的适应度
        if PUN_F is not None:
            [allFitnVs, exIdx] = punishing(allChroms, allFitnVs) # 调用罚函数
        # 记录进化过程
        bestIdx = np.argmax(allFitnVs)
        wrongSign = np.ones((allFitnVs.shape[0], 1))
        wrongSign[list(exIdx)] = 0 # 对非可行解作标记
        if allFitnVs[bestIdx] != 0:
        ...
        for index in range(len(FieldDRs)):
            ...
            Chroms[index] = ga.selecting(selectStyle, Chrom, FitnV, GGAP, SUBPOP) # 选择
