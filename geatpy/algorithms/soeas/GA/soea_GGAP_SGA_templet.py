...
class soea_GGAP_SGA_templet(ea.SoeaAlgorithm):
    ...
    def reinsertion(self, population, offspring, GGAP_NUM):
        """重插入."""
        replaceIdx = np.argsort(population.FitnV.T[0])[:GGAP_NUM].astype(
            int)  # 计算父代中要被替换的个体索引
        insertIdx = np.argsort(-offspring.FitnV.T[0])[:GGAP_NUM].astype(
            int)  # 计算子代中需要选择进行重插入的个体索引
    ...
    def run(self, prophetPop=None):  # prophetPop为先知种群（即包含先验知识的种群）
        ...
        while not self.terminated(population):
            ...
            offspring.FitnV = ea.scaling(offspring.ObjV,
                                         offspring.CV,
                                         self.problem.maxormins)  # 计算适应度
